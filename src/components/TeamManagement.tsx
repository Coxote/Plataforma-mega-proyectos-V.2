    ...
    <div className="p-4 sm:p-8 bg-[#F4F5F0] w-full min-h-full space-y-6 flex flex-col relative" id="team-management-panel">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-stone-200/60 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 font-normal">Asigna roles técnicos, controla cargas operativas y configura accesos de clientes.</p>
        </div>
        ...
        <div className="flex items-center gap-1 bg-white p-1.5 rounded-full shadow-xs self-start shrink-0">
          ...
          >
            <Grid className="w-3.5 h-3.5 text-slate-800" />
            Fichas Operativas
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-white p-6 rounded-3xl shadow-xs flex items-center gap-4 border border-stone-200/70">
          <div className="w-12 h-12 bg-stone-100 rounded-2xl flex items-center justify-center text-slate-800 shrink-0">
            <Users className="w-5 h-5 text-slate-800" />
          </div>
          <div>
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Escuadrón Interno</span>
            <span className="text-2xl sm:text-3xl font-semibold font-display text-slate-900">{totalMembers} Operadores</span>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-xs flex items-center gap-4 border border-stone-200/70">
          ...
          <div>
            ...
            <span className="text-2xl sm:text-3xl font-semibold font-display text-slate-900">{totalClients} Clientes</span>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-xs flex items-center gap-4 border border-stone-200/70">
          ...
          <div>
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Estado del Equipo</span>
            <span className="text-2xl sm:text-3xl font-semibold font-display text-slate-900">Sincronizado</span>
          </div>
        </div>
      </div>
      ...
      {subView === 'cards' ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 bg-white rounded-3xl p-6 shadow-xs space-y-4 self-start">
            ...
            <form onSubmit={handleCreateUser} className="space-y-4">
              {formError && (
                <div className="bg-rose-50 text-rose-700 text-xs rounded-2xl p-3 font-semibold">
                  {formError}
                </div>
              )}
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-[#F4F5F0] hover:bg-stone-100 focus:bg-white border-0 rounded-2xl pl-10 pr-3 py-2.5 text-xs text-slate-800 focus:ring-2 focus:ring-lime-400/50 outline-none transition-all font-medium"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    className="w-full bg-[#F4F5F0] hover:bg-stone-100 focus:bg-white border-0 rounded-2xl pl-10 pr-3 py-2.5 text-xs text-slate-800 focus:ring-2 focus:ring-lime-400/50 outline-none transition-all font-medium cursor-pointer"
                  >
              ...
              {newPuesto === 'Cliente / Invitado' && (
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className="w-full bg-[#F4F5F0] hover:bg-stone-100 focus:bg-white border-0 rounded-2xl px-3.5 py-2.5 text-xs text-slate-800 focus:ring-2 focus:ring-lime-400/50 outline-none transition-all font-medium cursor-pointer"
                  >
              ...
              <button
                type="submit"
                className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 rounded-full text-xs transition-colors cursor-pointer flex items-center justify-center gap-1.5 mt-2 shadow-xs"
              >
          ...
          <div className="lg:col-span-2 bg-white rounded-3xl p-6 shadow-xs space-y-4 overflow-hidden flex flex-col">
                      ...
                      <tr key={user.id} className="hover:bg-stone-50/70 transition-colors">
                        <td className="py-3 px-3 font-semibold text-slate-800">
                          <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-full bg-[#F4F5F0] flex items-center justify-center font-bold text-xs text-slate-700 capitalize shadow-2xs">
                        ...
                        <td className="py-3 px-3">
                          <div className="flex flex-col gap-1.5 max-w-[180px]">
                            <select
                              ...
                              disabled={isSelf}
                              className={`bg-[#F4F5F0] rounded-xl px-2.5 py-1 text-xs text-slate-850 outline-none transition-all cursor-pointer font-bold ${
                            ...
                            {user.role === 'invitado' && (
                              <select
                                ...
                                className="bg-[#F4F5F0] rounded-xl px-2 py-1 text-xs text-slate-700 outline-none transition-all cursor-pointer font-semibold"
                              >
      ...
      <CustomModal
        ...
        description={`¿Estás seguro de que deseas eliminar a "${userToDelete?.username}" (${userToDelete?.role})? Esta acción no se puede deshacer.`}
