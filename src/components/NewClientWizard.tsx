  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col">
        ...
        {/* HEADER */}
        <div className="flex justify-between items-center p-6 border-b border-stone-100 relative">
          ...
          <button onClick={handleResetAndClose} className="p-2 text-slate-400 hover:text-slate-900 bg-[#F4F5F0] hover:bg-stone-200 rounded-full transition-colors cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="col-span-2 md:col-span-1">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-[#F4F5F0] rounded-2xl text-sm font-bold focus:outline-none focus:ring-2 focus:ring-cyan-500/20 disabled:opacity-60"
                />
              </div>
              <div className="col-span-2 md:col-span-1">
              ...
              <div className="col-span-2">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Contacto Principal *</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-3 bg-[#F4F5F0] rounded-2xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-cyan-500/20 disabled:opacity-60"
                  />
                </div>
              </div>
              <div className="col-span-2 md:col-span-1">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-[#F4F5F0] rounded-2xl text-sm font-medium disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-cyan-500/20"
                />
              </div>
              <div className="col-span-2 md:col-span-1">
              ...
              <div className="col-span-2">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Sitio Web o Redes</label>
                <div className="relative">
              ...
              <div className="ml-auto">
                {isEditing ? (
                  <button
                    ...
                    onClick={() => setStep(2)}
                    className="px-6 py-3 bg-slate-900 hover:bg-black text-white rounded-full font-bold text-sm disabled:opacity-30 flex items-center gap-2 cursor-pointer transition-all shadow-xs"
                  >
                  ...
                  <button
                    onClick={onClose}
                    className="px-6 py-3 bg-slate-900 hover:bg-black text-white rounded-full font-bold text-sm cursor-pointer transition-all shadow-xs"
                  >
                    Cerrar Perfil
                  </button>
                )}
          ...
          <div className="p-8 space-y-6">
            {!brandBible ? (
              ...
              <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
                <div className="bg-slate-900 p-6 rounded-3xl text-white space-y-4 shadow-xs">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      <span className="block text-xs text-slate-400 uppercase font-bold">Arquetipo</span>
                      <p className="font-semibold text-slate-100">{brandBible.archetype}</p>
                    </div>
                    <div>
                      <span className="block text-xs text-slate-400 uppercase font-bold">Misión & Visión</span>
                      <p className="font-medium text-slate-200">{brandBible.misionVision}</p>
                    </div>
                    <div>
                      <span className="block text-xs text-slate-400 uppercase font-bold">Tono y Voz</span>
                      <p className="font-medium text-slate-200">{brandBible.tonoVoz}</p>
                    </div>
                    <div>
                      <span className="block text-xs text-slate-400 uppercase font-bold">Mensajes Clave</span>
                      <p className="font-semibold text-cyan-300 bg-slate-800/80 p-3 rounded-2xl text-xs">
                        {brandBible.mensajesClave}
                      </p>
                    </div>
                    <div>
                      <span className="block text-xs text-slate-400 uppercase font-bold mb-1.5">Colores Extraídos</span>
                      <div className="flex flex-wrap gap-2">
                        {brandBible.coloresHex?.map(color => (
                          <div key={color} className="flex items-center gap-1.5 bg-slate-800 pr-3 py-1 rounded-full text-xs">
                            <span className="w-5 h-5 rounded-full block border border-slate-700 shadow-2xs" style={{ backgroundColor: color }}></span>
                ...
                {isEditing && (
                  <div className="bg-[#F4F5F0] p-4 rounded-2xl flex items-center justify-between text-xs">
                    <span className="text-slate-600 font-medium">¿Quieres re-analizar otro archivo?</span>
                    <button
                      onClick={() => setBrandBible(null)}
                      className="px-3.5 py-1.5 bg-white hover:bg-stone-100 text-cyan-700 font-bold rounded-full transition-colors cursor-pointer shadow-2xs"
                    >
                      Cargar nuevo
                    </button>
                  </div>
