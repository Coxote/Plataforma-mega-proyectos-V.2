...
interface NewClientWizardProps {
  ...
  initialData?: Client | null; // <-- Para el modo ediciÃ³n
}
...
export const NewClientWizard: React.FC<NewClientWizardProps> = ({ isOpen, onClose, onSaveClient, initialData }) => {
  ...
  const processFile = (file: File) => {
    setIsAnalyzing(true);
    // SimulaciÃ³n de anÃ¡lisis IA leyendo el archivo
    setTimeout(() => {
      setBrandBible({
        archetype: 'El Creador / El Mago',
        misionVision: `Posicionarse como lÃ­deres indiscutibles en el sector ${categoria || 'digital'}.`,
        tonoVoz: 'Irreverente, directo, millenial y fresco.',
        coloresHex: ['#000000', '#06b6d4', '#10b981', '#f8fafc'],
        mensajesClave: 'Menos teorÃ­a, mÃ¡s conversiones aceleradas.'
      });
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xs z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col">
          ...
          <div className="flex items-center gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1 font-medium">
                {step === 1 ? 'Fase 1: Datos BÃ¡sicos' : 'Fase 2: Brand Bible IA'}
              </p>
            </div>
            {/* BOTÃ“N EDITAR EN LA PARTE SUPERIOR */}
        ...
        {step === 1 && (
          <div className="p-8 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="col-span-2 md:col-span-1">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">CategorÃ­a *</label>
                <input
                  type="text"
                  disabled={!isEditing}
                  value={categoria}
                  placeholder="Ej: Retail, TecnologÃ­a..."
              ...
              <div className="col-span-2 md:col-span-1">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">TelÃ©fono</label>
                <input
                  type="tel"
                  disabled={!isEditing}
                  value={telefono}
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  <span className="text-sm font-bold text-cyan-900 block mb-1">
                    {isEditing ? 'Arrastra el manual de marca o PDF aquÃ­' : 'No se ha cargado manual de marca'}
                  </span>
                  <span className="text-xs text-cyan-600/70 font-medium">
                    {isEditing ? 'o haz clic para explorar en tu equipo' : 'Ponte en modo ediciÃ³n para cargar un archivo'}
                  </span>
                </form>

                {isAnalyzing && (
                  <div className="text-center py-4 text-cyan-600 font-bold animate-pulse flex items-center justify-center gap-2">
                    <Sparkles className="w-5 h-5" /> La IA estÃ¡ analizando detalladamente el documento...
                  </div>
                )}
              </div>
            ) : (
              <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
                <div className="bg-slate-900 p-6 rounded-2xl text-white space-y-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="font-bold text-xs text-cyan-400 uppercase tracking-wider">Brand Bible ExtraÃ­do con Ã‰xito</h3>
                  </div>
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <div>
                      <span className="block text-xs text-slate-400 uppercase font-bold">MisiÃ³n & VisiÃ³n</span>
                    ...
                    <div>
                      <span className="block text-xs text-slate-400 uppercase font-bold mb-1.5">Colores ExtraÃ­dos</span>
                ...
                {isEditing && (
                  <div className="border border-dashed border-cyan-100 bg-cyan-50/20 p-4 rounded-2xl flex items-center justify-between text-xs">
                    <span className="text-slate-500 font-medium">Â¿Quieres re-analizar otro archivo?</span>
                    <button
