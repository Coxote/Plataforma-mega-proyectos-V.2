        ...
        <div className="bg-slate-900 text-white p-5 flex items-center justify-between border-b border-slate-800 relative overflow-hidden">
          <div className="flex items-center gap-2.5 z-10">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">AcciÃ³n global de carga de horas y retrabajo</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {isSuccessToast && (
            <div className="p-3 bg-emerald-50 text-emerald-800 rounded-2xl border border-emerald-200 flex items-center gap-2 text-xs font-bold animate-in slide-in-from-top duration-200">
              ...
              Â¡Horas registradas exitosamente en el expediente!
            </div>
          )}
          ...
          {phases.length > 0 && (
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1 flex items-center gap-1">
                <Layers className="w-3.5 h-3.5 text-cyan-500" /> Fase de EjecuciÃ³n
              </label>
              <select
          ...
          {/* CAMPOS ESPECÃFICOS DE RETRABAJO */}
          {entryType === 'retrabajo' && (
            <div className="p-4 bg-amber-50 rounded-2xl border border-amber-200 space-y-3">
              ...
              <div>
                <label className="block text-xs font-bold text-amber-900 uppercase mb-1">Motivo / Causa RaÃ­z *</label>
                <input
                  type="text"
          ...
          {/* CANTIDAD DE HORAS Y DESCRIPCIÃ“N */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Horas a Cargar *</label>
              <input
                type="number"
                step="0.5"
                placeholder="Ej: 3.5"
            ...
            <div className="sm:col-span-2">
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">
                {entryType === 'retrabajo' ? 'DescripciÃ³n Breve' : 'DescripciÃ³n de la Tarea *'}
              </label>
              <input
                type="text"
                placeholder="Ej: Ajuste de diseÃ±o de piezas grÃ¡ficas"
                value={description}
