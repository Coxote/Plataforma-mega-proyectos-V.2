...
export const ClientPortal: React.FC<ClientPortalProps> = ({ project, onAddAnnotation, onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-[#F4F5F0] flex flex-col">
      {/* Header */}
      <header className="bg-white py-4 px-6 sticky top-0 z-10 shadow-xs">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-slate-900 rounded-2xl flex items-center justify-center font-semibold text-white shadow-xs">
              S
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold text-slate-800 uppercase tracking-widest bg-stone-100 px-2.5 py-0.5 rounded-full">
                  Portal de Cliente
                </span>
                <span className="text-xs text-slate-400 font-normal">• Acceso Autorizado</span>
              </div>
              <h1 className="text-lg font-semibold text-slate-900 leading-tight">{project?.name || 'Proyecto de Marca'}</h1>
            </div>
          </div>

          <div className="flex items-center gap-4">
            <div className="hidden sm:block text-right">
              <p className="text-xs font-semibold text-slate-700">{project?.clientContact || 'Contacto Principal'}</p>
              <p className="text-xs text-slate-400 font-normal">{project?.clientName || 'Cliente'}</p>
            </div>

            <button
              onClick={onLogout}
              className="flex items-center gap-2 text-xs font-semibold text-slate-600 hover:text-rose-600 bg-[#F4F5F0] hover:bg-rose-50 px-4 py-2 rounded-full transition-all cursor-pointer shadow-xs"
            >
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-4 sm:p-6 space-y-6">
        {/* Project Intro Panel */}
        <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xs relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-stone-100 rounded-full blur-2xl pointer-events-none" />
          <div className="max-w-3xl space-y-3">
            <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Acerca del Proyecto</h2>
            <h3 className="text-2xl font-semibold text-slate-900 tracking-tight">Seguimiento de Entregables</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3">
              <div className="p-4 bg-[#F4F5F0] rounded-2xl space-y-1">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Objetivo Clave</span>
                <p className="text-xs text-slate-700 font-semibold">{project?.objective || 'Definido al iniciar'}</p>
              </div>
              <div className="p-4 bg-[#F4F5F0] rounded-2xl space-y-1">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Alcance Acordado</span>
                <p className="text-xs text-slate-700 font-semibold">{project?.alcance || 'Establecido en contrato'}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between border-b border-stone-200 pb-3">
            ...
            <span className="text-xs text-slate-400 font-normal">Total de archivos publicados</span>
          </div>

          {visibleDeliverables.length === 0 ? (
            <div className="bg-white rounded-3xl p-12 text-center shadow-xs flex flex-col items-center justify-center space-y-3">
              ...
              <div>
                <h3 className="font-semibold text-slate-800 text-sm">No hay entregables disponibles</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {visibleDeliverables.map((item) => (
                <div key={item.id} className="bg-white rounded-3xl p-6 shadow-xs flex flex-col justify-between hover:shadow-md transition-all space-y-4">

                  {/* File Metadata */}
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold uppercase px-3 py-1 bg-[#F4F5F0] text-slate-700 rounded-full tracking-wider flex items-center gap-1.5">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-slate-400 font-normal">Subido por: <strong className="text-slate-600 font-semibold">{item.uploadedBy || 'Coordinador'}</strong></p>
                    </div>
                    ...
                    {(item.externalUrl || item.fileUrl) && (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 text-xs text-slate-900 font-semibold hover:bg-stone-200 bg-stone-100 px-3.5 py-1.5 rounded-full transition-all"
                      >
                  ...
                  <div className="border-t border-stone-100 pt-4 space-y-3">
                    ...
                    <div className="max-h-36 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                      {(!item.annotations || item.annotations.length === 0) ? (
                        <p className="text-xs text-slate-400 italic py-2">No hay comentarios en este entregable. Agrega uno abajo.</p>
                      ) : (
                        item.annotations.map((ann) => (
                          <div key={ann.id} className="bg-[#F4F5F0] p-3 rounded-2xl text-xs space-y-1">
                            <div className="flex justify-between items-center text-xs text-slate-400">
                              ...
                              <div className="flex items-center gap-1.5">
                                ...
                                {ann.status === 'resuelto' ? (
                                  <span className="text-2xs font-extrabold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full flex items-center gap-0.5">
                                    <CheckCircle2 className="w-2.5 h-2.5" /> Resuelto
                                  </span>
                                ) : (
                                  <span className="text-2xs font-extrabold text-amber-700 bg-amber-100 px-2 py-0.5 rounded-full">
                                    Pendiente
                                  </span>
                                )}
                              </div>
                    ...
                    <div className="flex gap-2 pt-2">
                      <input
                        ...
                        className="flex-1 bg-[#F4F5F0] rounded-2xl px-4 py-2.5 text-xs outline-none focus:ring-2 focus:ring-lime-500 focus:bg-white transition-all placeholder:text-slate-400"
      ...
      {/* Footer */}
      <footer className="bg-white py-6 px-6 mt-12 text-center text-xs text-slate-400">
