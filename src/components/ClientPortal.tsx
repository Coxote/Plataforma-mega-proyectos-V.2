  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex flex-col">
        ...
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-lime-700 uppercase tracking-widest bg-lime-100 px-2 py-0.5 rounded-md">
                  Portal de Cliente
                </span>
                <span className="text-xs text-slate-400">â€¢ Acceso Autorizado</span>
              </div>
          ...
          <div className="flex items-center gap-4">
            <div className="hidden sm:block text-right">
              ...
              <p className="text-xs text-slate-400">{project?.clientName || 'Cliente'}</p>
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs relative overflow-hidden">
          ...
          <div className="max-w-3xl space-y-2">
            ...
            <p className="text-sm text-slate-600 leading-relaxed">
              {project?.description || 'Bienvenido a tu portal personalizado. AquÃ­ podrÃ¡s revisar los entregables aprobados y listos para tu feedback en tiempo real. Utiliza la secciÃ³n de anotaciones debajo de cada tarjeta para informarnos sobre cualquier cambio necesario.'}
            </p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3">
              <div className="p-3 bg-slate-50 border border-slate-100 rounded-xl space-y-1">
                <span className="text-xs font-bold text-slate-400 uppercase">Objetivo Clave</span>
                <p className="text-xs text-slate-700 font-medium">{project?.objective || 'Definido al iniciar'}</p>
              </div>
              <div className="p-3 bg-slate-50 border border-slate-100 rounded-xl space-y-1">
                <span className="text-xs font-bold text-slate-400 uppercase">Alcance Acordado</span>
          ...
          <div className="flex items-center justify-between border-b border-slate-200 pb-2">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold text-slate-800 uppercase tracking-wider">
                ðŸ“¦ Entregables PÃºblicos para RevisiÃ³n ({visibleDeliverables.length})
              </h2>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-xs flex flex-col items-center justify-center space-y-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1 max-w-sm">
                  Su coordinador de proyectos no ha publicado ningÃºn archivo para revisiÃ³n en esta fase todavÃ­a.
                </p>
              </div>
            </div>
          ) : (
                ...
                <div key={item.id} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex flex-col justify-between hover:border-slate-300 hover:shadow-md transition-all space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-extrabold uppercase px-2 py-1 bg-slate-100 text-slate-600 rounded-md tracking-wider flex items-center gap-1.5">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-slate-400">Subido por: {item.uploadedBy || 'Coordinador'}</p>
                    </div>
                  ...
                  <div className="border-t border-slate-100 pt-4 space-y-3">
                          ...
                          <div key={ann.id} className="bg-slate-50 border border-slate-100 p-2.5 rounded-xl text-xs space-y-1">
                            <div className="flex justify-between items-center text-xs text-slate-400">
                              ...
                              <div className="flex items-center gap-1.5">
                                <span>{ann.date}</span>
                                {ann.status === 'resuelto' ? (
                                  <span className="text-xs font-bold text-emerald-600 bg-emerald-100 px-1 py-0.2 rounded-md flex items-center gap-0.5">
                                    <CheckCircle2 className="w-2 h-2" /> Resuelto
                                  </span>
                                ) : (
                                  <span className="text-xs font-bold text-amber-600 bg-amber-100 px-1 py-0.2 rounded-md">
                                    Pendiente
                                  </span>
                                )}
                              </div>
                    ...
                    <div className="flex gap-2 pt-2">
                      <input
                        type="text"
                        placeholder="Escribe una observaciÃ³n o solicita cambios..."
      ...
      <footer className="bg-white border-t border-slate-200 py-4 px-6 mt-12 text-center text-xs text-slate-400">
        <p>Â© 2026 {project?.clientName || 'SaaS Client Portal'}. Con tecnologÃ­a de Herramienta Interna de Fases.</p>
      </footer>
    </div>
  );
};
